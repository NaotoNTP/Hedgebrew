...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
		rsreset
oPrevDPLC	rs.b	0				; Previous DPLC ID (only used by a few objects)
oAddr		rs.l	1				; Pointer to object code
oNext		rs.w	1				; Pointer to next object in the linked list
...
oVRAM		rs.w	1				; Sprite tile properties
oFrame		rs.b	0				; Mapping frame ID (top byte of the following long)
oMap		rs.l	1				; Sprite mappings

oXPos		rs.l	1				; X position
oDrawW	=	__rs-1					; Sprite width
oYPos		rs.l	1				; Y position
oDrawH	=	__rs-1					; Sprite height
oXVel		rs.w	1				; X velocity
oYVel		rs.w	1				; Y velocity

oAni		rs.b	1				; Animation ID
oPrevAni	rs.b	1				; Saved animation ID
oAniFrame	rs.b	1				; Animation script frame ID
oAniTimer	rs.b	1				; Animation timer

oSubtype	rs.b	1				; Subtype ID (top byte of the following long)
oRoutine	rs.b	1				; Routine ID
oRespawn	rs.w	1				; Respawn table entry pointer
oStatus		rs.b	1				; Status flags
oShield		rs.b	1				; Shield flags

oDynSSTs	rs.b	$60-__rs			; Dynamic SSTs
oSize		rs.b	0				; Size of the SSTs
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
; Dynamic SSTs
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
		rsset	oDynSSTs
oColType	rs.b	1				; Collision type
oColStat	rs.b	1				; Collision status
oHitCnt		equ	oColStat			; Boss hit count
oColW		rs.b	1				; Collision width
oColH		rs.b	1				; Collision height
oNextTilt	rs.b	1				; Next tilt value
oTilt		rs.b	1				; Tilt value
oLvlSSTs	rs.b	0				; Beginning of dynamic level SSTs
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
; Sub sprite SSTs
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
		rsreset
oSubXPos	rs.w	1				; Sub sprite X position
oSubYPos	rs.w	1				; Sub sprite Y position
		rs.b	1				; Unused
oSubFrame	rs.b	1				; Sub sprite mapping frame
oSubSize	rs.b	0				; Sub sprite SSTs size
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
		rsset	oNextTilt			; Allow some reserved SSTs
oSubSSTs	rs.b	0				; Start of sub sprite SSTs
oSubCnt		rs.w	1				; Main sprite sub sprite count
oSubStart	rs.b	0				; Actual sub sprite SSTs start
ct		=	0
	rept	8					; Allow 8 sub sprites per object
oSub\$ct\X	rs.w	1				; Sub sprite X position
oSub\$ct\Y	rs.w	1				; Sub sprite Y position
		rs.b	1				; Unused
oSub\$ct\Frame	rs.b	1				; Sub sprite mapping frame position
ct		=	ct+1
	endr
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
; Sprite drawing input list definitions
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
		rsset oDrawPrev-6			; this awkward thing will make dPrev == oDrawPrev
...
rObj_\$curobj		rs.b	oSize
curobj_size		=	curobj_size+oSize
curobj			=	curobj+1
		endw
		endm
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
