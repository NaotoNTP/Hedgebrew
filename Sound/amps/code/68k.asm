	...
	include "Sound/amps/code/routines.asm"	; include macro'd routines
	include "Sound/amps/code/debug.asm"	; debug data blob
; ===========================================================================
; ---------------------------------------------------------------------------
...
PlayDAC2:
	...
	endr
		move.b	(a3)+,d0
		addq.b	#1,d0
		move.b	d0,dZ80+PCM2_PitchHigh+1
		move.b	(a3)+,dZ80+PCM2_PitchLow+1
		move.b	#$D2,dZ80+PCM2_ChangePitch; change "JP C" to "JP NC"

		move.b	#$DA,dZ80+PCM2_NewRET	; activate sample switch (change instruction)
...
PlayDAC1:
		lea	dZ80+PCM1_Sample,a1	; load addresses for PCM 1
	StopZ80					; wait for Z80 to stop
	rept 12
		move.b	(a3)+,(a1)+		; send sample data to Dual PCM
	endr
		move.b	(a3)+,d0
		addq.b	#1,d0
		move.b	d0,dZ80+PCM1_PitchHigh+1
		move.b	(a3)+,dZ80+PCM1_PitchLow+1
		move.b	#$D2,dZ80+PCM1_ChangePitch; change "JP C" to "JP NC"

		move.b	#$DA,dZ80+PCM1_NewRET	; activate sample switch (change instruction)
	StartZ80				; enable Z80 execution
		rts
; ===========================================================================
; ---------------------------------------------------------------------------
...
dAMPSdoFMSFX:
		lea	VoiceBankSFX(pc),a6	; load sound effects voice table into a6
...
dAMPSdoFM:
		lea	VoiceBankMusic(pc),a6	; load music voice table into a6
...
.dacdone
		...
		bset	#cfbRest,(a5)		; Set channel resting flag
		lea	VoiceBankMusic(pc),a1	; load music voice table to a1
		move.b	cVoice(a5),d0		; load FM voice ID of the channel to d0
...
dChkSongPlay:
	lea	mDAC1.w,a1		; Music track RAM
	moveq	#Mus_Ch-1,d1

.ChkTracks:
	tst.b	cFlags(a1)		; Is this track playing?
	bmi.s	.Playing		; If so, branch
	lea	cSize(a1),a1		; Next track
	dbf	d1,.ChkTracks		; Loop
	moveq	#0,d1
	rts
...
VoiceBankMusic:
sPatNum = 0
	include "Sound/unipatchmus.asm"; include universal Voice banks
VoiceBankSFX:
sPatNum = 0
	include "Sound/unipatchsfx.asm"; include universal Voice banks
	opt ae-				; disable automatic evens

sfxaddr	include	"Sound/sfxdata.asm"	; include all sfx
musaddr	include	"Sound/musicdata.asm"	; include all music
musend
; ===========================================================================
; ---------------------------------------------------------------------------
...
fLog:		incbin "Sound/amps/filters/Logarithmic.dat"	; logarithmic filter (no filter)
;fLinear:	incbin "Sound/amps/filters/Linear.dat"	; linear filter (no filter)
